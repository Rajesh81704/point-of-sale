...
const logoutAuthController = (req, res) => {
  ...
  const userId = [...refreshTokens.entries()].find(
...
const authRegisterController = async(req, res) => {
    ...
    try {
        ...
        const insertUserQuery = `
            ...
            RETURNING *`
        const insertUserResult = await client.query(insertUserQuery, [username, hashedPassword, email, phoneNo, additional_dtls, profile_image]);
        ...
        return res.status(201).json({ message: 'User registered successfully', user: insertUserResult.rows[0] });
    } catch (err) {
        await client.query('ROLLBACK');
...
export { logoutAuthController,authRegisterController ,authLoginController};